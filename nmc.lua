...
if string.find(data.Content, "中国降水量") then 
...
if string.find(data.Content, "西南雷达图") then 


        response, error_message =
            http.request(
            "GET",
            "http://www.nmc.cn/rest/category/98" --从官方json接口获取图像列表
        )
        local html = response.body
        local str = html:gsub("/medium","") --从返回包体中替换掉medium图片路径，转为原文件
	local image = json.decode(str) --反序列化json
...
if string.find(data.Content, "西北雷达图") then 


        response, error_message =
            http.request(
            "GET",
            "http://www.nmc.cn/rest/category/97" --从官方json接口获取图像列表
        )
        local html = response.body
        local str = html:gsub("/medium","") --从返回包体中替换掉medium图片路径，转为原文件
	local image = json.decode(str) --反序列化json
...
if string.find(data.Content, "华中雷达图") then 


        response, error_message =
            http.request(
            "GET",
            "http://www.nmc.cn/rest/category/96" --从官方json接口获取图像列表
        )
        local html = response.body
        local str = html:gsub("/medium","") --从返回包体中替换掉medium图片路径，转为原文件
	local image = json.decode(str) --反序列化json
...
if string.find(data.Content, "华南雷达图") then 


        response, error_message =
            http.request(
            "GET",
            "http://www.nmc.cn/rest/category/95" --从官方json接口获取图像列表
        )
        local html = response.body
        local str = html:gsub("/medium","") --从返回包体中替换掉medium图片路径，转为原文件
	local image = json.decode(str) --反序列化json
...
if string.find(data.Content, "华北雷达图") then 


        response, error_message =
            http.request(
            "GET",
            "http://www.nmc.cn/rest/category/94" --从官方json接口获取图像列表
        )
        local html = response.body
        local str = html:gsub("/medium","") --从返回包体中替换掉medium图片路径，转为原文件
	local image = json.decode(str) --反序列化json
...
if string.find(data.Content, "东北雷达图") then 


        response, error_message =
            http.request(
            "GET",
            "http://www.nmc.cn/rest/category/93" --从官方json接口获取图像列表
        )
        local html = response.body
        local str = html:gsub("/medium","") --从返回包体中替换掉medium图片路径，转为原文件
	local image = json.decode(str) --反序列化json
...
if string.find(data.Content, "全国雷达图") then 


        response, error_message =
            http.request(
            "GET",
            "http://www.nmc.cn/rest/category/92" --从官方json接口获取图像列表
        )
        local html = response.body
        local str = html:gsub("/medium","") --从返回包体中替换掉medium图片路径，转为原文件
	local image = json.decode(str) --反序列化json
...
if string.find(data.Content, "副高") then --亚欧500hPa叠加卫星云图
